...
with middle_panel:
    with st.container(border=True):
        ...
        video_sources = [
            (tr("Pexels"), "pexels"),
            (tr("Pixabay"), "pixabay"),
            (tr("Image Search (Wikipedia + Pixabay Photos)"), "image_search"),
            (tr("Local file"), "local"),
            (tr("TikTok"), "douyin"),
        ...
        if params.video_source == "local":
            ...
            st.info("📸 Image Search: DuckDuckGo (free) + Pexels Photos + Wikipedia Commons. Best for sports history, news events, real people.")
            uploaded_files = []
            ...
            with st.container(border=True):
                ...
                params.enable_image_ranking = st.checkbox(
                    "🧠 CLIP Image Ranking (match images to script sentences)",
                    value=True,
                    help="Uses AI to put the most relevant image under each part of the narration. Requires torch + transformers."
                )

                st.write("**Hook Card** (first 1.5s scroll-stopper)")
                params.enable_hook_card = st.checkbox("Enable Hook Card", value=True)
                if params.enable_hook_card:
                    params.hook_text = st.text_input(
                        "Hook Text (leave blank to auto-generate)",
                        value="",
                        placeholder="e.g. HE ENDED HIS CAREER WITH THIS",
                        help="3-6 ALL-CAPS words. Auto-generated by LLM if left blank."
                    )
                    params.hook_duration = st.slider("Hook Duration (s)", 0.8, 3.0, 1.5, 0.1)

                st.write("**End-screen CTA** (last 2s)")
                params.enable_cta = st.checkbox("Enable CTA Overlay", value=True)
                if params.enable_cta:
                    params.cta_text = st.text_input("CTA Text", value="FOLLOW FOR MORE")

                params.enable_sfx = st.checkbox(
                    "🔊 SFX on image cuts (place whoosh.mp3 / boom.mp3 in resource/sfx/)",
                    value=True,
                )
                if params.enable_sfx:
                    params.sfx_volume = st.slider("SFX Volume", 0.1, 1.0, 0.5, 0.05)
        else:
            uploaded_files = []
...
if start_button:
    ...
    if params.video_source not in ["pexels", "pixabay", "local", "image_search"]:
